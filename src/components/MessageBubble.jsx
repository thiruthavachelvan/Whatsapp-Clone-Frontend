...
const MessageBubble = ({ message, isOwn, showTail }) => {
  ...
  return (
    <div className={`flex flex-col mb-[2px] relative w-full ${isOwn ? 'items-end' : 'items-start'}`}>
      <div 
        className={`max-w-[85%] sm:max-w-[70%] rounded-lg px-2 sm:px-3 py-1 sm:py-1.5 text-sm sm:text-[14.2px] break-words shadow-sm flex relative ${
          isOwn 
            ? 'bg-[#005c4b] dark:bg-[#d9fdd3] text-white dark:text-[#111b21]' 
            : 'bg-[#202c33] dark:bg-white text-white dark:text-[#111b21]'
      ...
      >
        ...
        {showTail && (
          <span 
            className={`absolute top-0 w-2 h-3.5 ${
              isOwn 
                ? '-right-2 text-[#005c4b] dark:text-[#d9fdd3]' 
                : '-left-2 text-[#202c33] dark:text-white'
            }`}
          >
            {isOwn ? (
        ...
        <div className="flex flex-col w-full pr-10 min-w-0">
          ...
          <div className="absolute bottom-[2px] right-1.5 flex items-center">
            <span className={`text-[10px] tracking-tight whitespace-nowrap ml-2 ${isOwn || !isOwn ? 'opacity-70' : ''}`}>
              {timeString}
            </span>
            {isOwn && (
              <span className={`ml-[2px] scale-90 ${message.isRead ? 'text-[#53bdeb]' : 'text-white/60 dark:text-[#8696a0]'}`}>
